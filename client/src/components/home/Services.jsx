        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featuredServices.map((service, index) => (
            ...
            >
              ...
              <div className="p-6">
                <p className="text-gray-600 mb-4 line-clamp-2 text-sm">{service.description.split('.')[0]}.</p>
                
                {service.applications && service.applications.length > 0 && (
                  <div className="mb-4 grid grid-cols-2 gap-2">
                    {service.applications.slice(0, 4).map((app, idx) => (
                      <div key={idx} className="flex items-center">
                        <img src={app.icon} alt={app.title} className="w-6 h-6 mr-2" />
                        <span className="text-xs text-gray-700 truncate">{app.title}</span>
                      </div>
                    ))}
                  </div>
                )}
                
                {!service.applications && service.features && service.features.length > 0 && (
